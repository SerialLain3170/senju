    ...
    def __init__(self,
        ...
        if sn:
            self.fc = nn.Sequential(
                spectral_norm(EqualizedLinear(base*8*3*4, base*8)),
                nn.LeakyReLU(),
                spectral_norm(EqualizedLinear(base*8, 1))
            )
        else:
            self.fc = nn.Sequential(
                EqualizedLinear(base*8*3*4, base*8),
                nn.LeakyReLU(),
    ...
    def __init__(self,
        ...
        if sn:
            self.linear = nn.Sequential(
                spectral_norm(EqualizedLinear(base*12 * 1 * 1, base*32)),
                nn.LeakyReLU(),
            ...
            self.linear = nn.Sequential(
                EqualizedLinear(base*12 * 1 * 1, base*32),
                nn.LeakyReLU(),
